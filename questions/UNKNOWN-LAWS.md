...
### D11 — Reconciling the residue: one structure, three descriptions

**Engine:** gap prediction — [`SYNTHESIS.md`](../SYNTHESIS.md) §7.0(iii) flagged
this explicitly as the register's next question, after a branch-reconciliation
pass turned up three independent accounts of "what the residue is in general"
that had never been checked against each other.
**Novelty: N1 claimed** (recombination of known results, correctly composed —
see the prior-art note, which names the exact test that would demote even the
one derived consequence to N0).
**Status: ⟳ RUN — all four registered predictions pass, three to
...
> **⟳ Restraint pass: the three accounts are not competing — they are one
> structure (a description map's linearization at a fixed point) recovered
> three times, plus one boundary strong enough to prove rather than only
> observe.**
>
> **D7/D8's residue is the projectivized eigenvalue vector of that
> linearization**, made literal: when a family of systems is generated by
> repeated application of a description map T with fixed point x*, log-slopes
> of observables against the approach parameter are (up to the common scalar
> D7 already isolated) eigenvalues of DT(x*) restricted to the directions the
> observables probe. This is an identity given D7's own hypotheses, not a new
> claim — D2's own R1–R3 legs already were RG eigenvalues read as critical
> exponents; this names what they, D7 and D8 had in common.
>
> **D3-ladder-invariance's "fixed point, ratio as eigenvalue" is the
> literature's own frame for exactly this**, not a fourth construction:
> stable laws and the Gaussian are fixed points of a renormalized-convolution
> operator (Jona-Lasinio 2001; Calvo–Cuchí–Esteve–Falceto 2010), which is
> literally the map D3's own aggregation harness iterates without naming.
>
> **Proved, not merely observed:** for that map, *any* law of U makes the
> scale mixture X = U·Z an exact fixed point (S_n =_d √n·X for every n),
> giving an explicit, infinite-dimensional continuum of genuinely distinct
> laws sharing one eigenvalue (H = 1/2). Measured to confirm the estimator
> — not the identity, which is proved — faithfully recovers a known exact
> value once Monte Carlo noise is removed: **every deviation from 0.5 is at
> the machine-epsilon floor (≤ 1.7e-16)** across 5 mixture configurations and
> 2 observables, registered tolerance 1e-6. **The falsifiable core:** a
> scale-invariant shape statistic — by construction exactly the direction
> tangent to the fixed-point set, hence predicted invisible to the residue —
> varies by **208%** across the same 5 configurations (registered bar: 15%)
> while staying exactly n-invariant (≤ 1.1e-14) throughout, targeted in
> advance rather than found post hoc as D3's original counterexample was.
>
> **D10 is not a fourth account.** It answers a different question — which
> variables carry an eigenvalue structure at all — checked here on a variable
> (an aggregate count) the register had not applied it to: additive under
> concatenation, hence pinned (G_diff), hence the shared exponent is a *fact*
> here, matching [D3-ladder-as-quotient](../experiments/D3-ladder-as-quotient/)'s
> independent finding on the same harness.
>
> **Honest scope.** One setting only: a single control, a linear (exactly
> solvable) description map. D8's m > 1 Grassmannian case and a description
> map with genuine nonlinear dynamics are both untested, named as the next
> probes. Graded **N1**: the RG-eigenvalue/exponent identification and the
> CLT-as-RG-fixed-point picture are both textbook (prior-art note, written
> first); what is not previously stated is the specific cross-connection of
> this repo's own D7/D8/D10/D3 results, and the closed-form proof that the
> residue's blindness is a general boundary of the construction rather than
> a property of D3's one accidental example.

**The claim.** A "description map" T on a space of systems, with a fixed
point x* (T(x\*) = x\*). Near x\*, the approach is governed by the
linearization DT(x\*), whose eigenvalues split into relevant, irrelevant and
marginal by the standard RG classification. **The tower's residue is the
projectivized eigenvalue vector of DT(x\*)** — D7/D8's ray or Grassmannian
point exactly — **on whichever variables D10's composition-pinning criterion
leaves free to re-chart.** Because it is built from log-slopes alone, the
residue is provably blind to any direction *tangent to the fixed-point set*:
a parameter that moves between fixed points without touching the leading
eigenvalue contributes nothing to any observable's log-slope, by definition.
Where the fixed-point set is a point (the generic floor-3 germ case), this
costs nothing. Where it has positive-dimensional tangent directions — a
"moduli" direction, in the two nearest physics analogues named in the
prior-art note (lines of fixed points from a marginal operator; CFT conformal
manifolds) — the residue is necessary but never sufficient to determine the
fixed point, and no enlargement of the log-slope construction (more
same-kind observables against the same control) can close that gap; only a
transverse measurement (comparing family members directly) can.

**Prior-art note.** Every load-bearing piece is textbook: RG eigenvalues as
critical exponents (Wilson, Kadanoff, Wegner — already D2's own prior art);
the CLT and stable laws as fixed points of a renormalized-convolution
operator (Jona-Lasinio, *Renormalization group and probability theory*,
Phys. Rep. 352 (2001) 439; Calvo, Cuchí, Esteve & Falceto, *Generalized
Central Limit Theorem and Renormalization Group*, J. Stat. Phys. 141 (2010)
409); continua of fixed points sharing invariant data while the underlying
theory differs, under two names that are close but not exact matches — lines
of fixed points from an exactly marginal operator (XY model, eight-vertex
model; here the *exponents themselves* vary along the line, unlike this
stone's construction), and CFT conformal manifolds / exactly marginal
deformations (protected scaling dimensions fixed while OPE data varies —
closer in spirit, but normally a supersymmetry fact, and this construction
needs none). **Nearest-miss test:** if a source states, as a general
cross-domain claim rather than a model-specific remark, that an approach's
exponent data is exactly the reparameterization-invariant content of a
fixed-point linearization and is generically insufficient to determine the
fixed point because of tangent directions the exponents cannot see — this
stone is N0 and only the specific four-way identification survives.

**Falsifier — inherited from the construction, not newly at risk here.** Any
of D2/D6/D7/D8/D10/D3's own falsifiers firing would undercut a premise this
stone assembles rather than re-tests. The one falsifier genuinely local to
D11: a description map with a fixed-point set whose tangent directions *do*
show up in some log-slope-built observable — which would mean the "tangent
direction" boundary is not general, only true of the one family tested.
